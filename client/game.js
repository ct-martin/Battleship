...
const redrawBase = () => {
  ...
  game.drawInfo.cellSize =  game.drawInfo.boardSize / 11.1; // 10 rows/cols + header row/col + padding*2
  game.drawInfo.cellPadding = (game.drawInfo.cellSize * 11.1 / 11.0) * .05; // use 5% of cellSize as padding on each side after unsetting board padding
...
const drawBoardCell = (row, col) => {
  ...
  if(game.displaySetting === game.ENUM.DISPLAY_SETTING['DUALTOUCH_TOP']) {
    if(game.boards.hits.includes(cellCanonical)) {
      ...
      if(mouseWasClicked && isMouseHovering) {
